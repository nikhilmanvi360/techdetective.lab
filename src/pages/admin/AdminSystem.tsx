  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
      <div className="space-y-6">
        ...
        {adversaryConfig && (
          <div className="bg-black/60 border border-cyber-red/30 p-6 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                className={`px-4 py-1 font-display font-bold uppercase text-xs border ${adversaryConfig.enabled ? 'border-cyber-green text-cyber-green' : 'border-cyber-red text-cyber-red'}`}
              >
        ...
        <div className="bg-black/60 border border-cyber-red/30 p-6 space-y-4">
          <h3 className="text-xs font-display font-bold text-cyber-red uppercase border-b border-cyber-red/20 pb-2">Manual Hack Deploy</h3>
          <form className="space-y-4 font-mono text-xs" onSubmit={async e => {
            ...
            <select name="actionType" className="w-full bg-black border border-cyber-line p-2 text-white">
            ...
            <input required name="message" type="text" placeholder="Payload message..." className="w-full bg-black border border-cyber-line p-2 text-white" />
            <button className="w-full bg-cyber-red/20 text-cyber-red font-bold py-2 border border-cyber-red hover:bg-cyber-red hover:text-black transition-colors">EXECUTE PAYLOAD</button>
          </form>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-black/60 border border-cyber-amber/30 p-6">
          <form onSubmit={handleSaveMul} className="space-y-4 font-mono text-xs">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input required name="multiplier" type="number" step="0.1" placeholder="e.g. 1.5" className="w-full bg-black border border-cyber-line p-2 text-white" />
              </div>
              <div className="space-y-2">
                <label className="text-gray-500 uppercase">Duration (Mins)</label>
                <input required name="durationMinutes" type="number" placeholder="e.g. 15" className="w-full bg-black border border-cyber-line p-2 text-white" />
              </div>
            </div>
            <button className="w-full bg-cyber-amber/20 text-cyber-amber font-bold py-2 border border-cyber-amber hover:bg-cyber-amber hover:text-black transition-colors">ACTIVATE XP BOOST</button>
          </form>
        </div>

        <div className="space-y-2">
          {multipliers.filter(m => new Date(m.expires_at) > new Date()).map((m, i) => (
             <div key={i} className="p-3 border border-cyber-amber/30 bg-cyber-amber/10 flex justify-between items-center text-xs font-mono text-white">
                <span className="flex items-center gap-2"><Zap className="w-3 h-3 text-cyber-amber fill-cyber-amber"/> {m.multiplier}X ACTIVE</span>
                <span className="text-cyber-amber">{new Date(m.expires_at).toLocaleTimeString()}</span>
             </div>
          ))}
        </div>
      </div>
