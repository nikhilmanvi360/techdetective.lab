    ...
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-black/80 border border-purple-500/30 p-6 space-y-6">
          <div className="flex items-center gap-3 border-b border-cyber-line pb-3">
          ...
          <form onSubmit={handleCreateCase} className="space-y-4 font-mono text-xs">
            <input required type="text" placeholder="Title" value={newCase.title} onChange={e => setNewCase({...newCase, title: e.target.value})} className="w-full bg-black border border-cyber-line p-2 text-white" />
            <textarea required rows={3} placeholder="Mission Briefing..." value={newCase.description} onChange={e => setNewCase({...newCase, description: e.target.value})} className="w-full bg-black border border-cyber-line p-2 text-white" />
            <div className="grid grid-cols-2 gap-4">
              <select value={newCase.difficulty} onChange={e => setNewCase({...newCase, difficulty: e.target.value})} className="bg-black border border-cyber-line p-2 text-white">
                <option>Easy</option><option>Intermediate</option><option>Hard</option>
              </select>
              <input required type="number" placeholder="XP Reward" value={newCase.points_on_solve} onChange={e => setNewCase({...newCase, points_on_solve: +e.target.value})} className="bg-black border border-cyber-line p-2 text-white" />
            </div>
        ...
        {/* Evidence Builder */}
        <div className="bg-black/80 border border-cyber-blue/30 p-6 space-y-6">
          <div className="flex items-center gap-3 border-b border-cyber-line pb-3">
          ...
          <form onSubmit={handleCreateEvidence} className="space-y-4 font-mono text-xs">
            <select required value={newEvidence.case_id} onChange={e => setNewEvidence({...newEvidence, case_id: e.target.value})} className="w-full bg-black border border-cyber-line p-2 text-white">
              <option value="">Select Parent Incident...</option>
              {masterKey.map(k => <option key={k.id} value={k.id}>{k.title}</option>)}
            </select>
            <div className="grid grid-cols-2 gap-4">
              <select value={newEvidence.type} onChange={e => setNewEvidence({...newEvidence, type: e.target.value})} className="bg-black border border-cyber-line p-2 text-white">
                <option value="log">Server Log</option><option value="chat">Comms Trace</option><option value="html">System Hex</option>
              </select>
              <input required type="text" placeholder="Title" value={newEvidence.title} onChange={e => setNewEvidence({...newEvidence, title: e.target.value})} className="w-full bg-black border border-cyber-line p-2 text-white" />
            </div>
            <textarea required rows={5} placeholder="Raw Telemetry Data..." value={newEvidence.content} onChange={e => setNewEvidence({...newEvidence, content: e.target.value})} className="w-full bg-black border border-cyber-line p-2 text-white" />
            <button type="submit" className="w-full bg-cyber-blue/20 text-cyber-blue font-bold uppercase tracking-widest py-3 border border-cyber-blue/50 hover:bg-cyber-blue hover:text-black transition-colors">Inject Data</button>
          </form>
        </div>

        {/* Puzzle Builder */}
        <div className="bg-black/80 border border-cyber-amber/30 p-6 space-y-6 lg:col-span-2 text-xs font-mono">
           <div className="flex items-center gap-3 border-b border-cyber-line pb-3">
          ...
          <form onSubmit={handleCreatePuzzle} className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="space-y-4">
                <select required value={newPuzzle.case_id} onChange={e => setNewPuzzle({...newPuzzle, case_id: e.target.value})} className="w-full bg-black border border-cyber-line p-2 text-white">
                  <option value="">Select Parent Incident...</option>
                  {masterKey.map(k => <option key={k.id} value={k.id}>{k.title}</option>)}
                </select>
                <input required type="text" placeholder="Question / Objective" value={newPuzzle.question} onChange={e => setNewPuzzle({...newPuzzle, question: e.target.value})} className="w-full bg-black border border-cyber-line p-2 text-white" />
             </div>
             <div className="space-y-4">
                <input required type="text" placeholder="Access Code (Answer)" value={newPuzzle.answer} onChange={e => setNewPuzzle({...newPuzzle, answer: e.target.value})} className="w-full bg-black border border-cyber-amber p-2 text-white" />
                <div className="grid grid-cols-2 gap-4">
                  <input required type="text" placeholder="Hint" value={newPuzzle.hint} onChange={e => setNewPuzzle({...newPuzzle, hint: e.target.value})} className="bg-black border border-cyber-line p-2 text-white" />
                  <input required type="number" placeholder="XP" value={newPuzzle.points} onChange={e => setNewPuzzle({...newPuzzle, points: +e.target.value})} className="bg-black border border-cyber-line p-2 text-white tabular-nums" />
                </div>
             </div>
             <div className="col-span-full">
                <button type="submit" className="w-full bg-cyber-amber/20 text-cyber-amber font-bold uppercase tracking-widest py-3 border border-cyber-amber/50 hover:bg-cyber-amber hover:text-black transition-colors">Plant Cipher</button>
             </div>
          </form>
        </div>
      </div>
