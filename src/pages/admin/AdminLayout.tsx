...
export default function AdminLayout({ team, onLogout }: AdminLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex bg-black">
      ...
      {/* ── Sidebar ── */}
      <aside className="w-64 border-r border-cyber-red/20 flex flex-col bg-black/90 relative z-10">
        {/* Sidebar glow */}
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(239,68,68,0.04) 0%, transparent 50%)' }} />

        {/* Branding */}
        <div className="p-6 border-b border-cyber-red/20 relative z-10">
          <div className="flex items-center gap-3 mb-1">
            <div className="p-2 bg-cyber-red/5 border border-cyber-red/30">
              <Shield className="w-5 h-5 text-cyber-red" />
            </div>
            <div>
              <div className="text-[10px] font-display text-cyber-red uppercase tracking-[0.3em]">CCU Admin</div>
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 transition-all group relative ${
                  active
                    ...
                    : 'border border-transparent text-gray-500 hover:text-white hover:border-cyber-line'
                }`}
              >
                {active && (
                  <motion.div
                    layoutId="admin-nav-active"
                    className="absolute left-0 top-0 bottom-0 w-[3px] bg-cyber-red"
                  />
                )}
                <span className={active ? 'text-cyber-red' : 'group-hover:text-white transition-colors'}>
                  {item.icon}
                </span>
                <span className="text-xs font-display uppercase tracking-widest">{item.label}</span>
                {active && <ChevronRight className="w-3 h-3 ml-auto text-cyber-red/50" />}
              </Link>
            );
          })}
        </nav>
        ...
        {/* Back to Game + Logout */}
        <div className="p-4 border-t border-cyber-red/10 space-y-2 relative z-10">
          <Link to="/" onClick={() => playSound('click')}
            className="flex items-center gap-3 px-4 py-2 text-gray-600 hover:text-white border border-transparent hover:border-cyber-line transition-all text-xs font-display uppercase tracking-widest">
            <Zap className="w-3 h-3" /> Back to Game
          </Link>
          <button onClick={handleLogout}
            className="w-full flex items-center gap-3 px-4 py-2 text-cyber-red/50 hover:text-cyber-red border border-transparent hover:border-cyber-red/30 transition-all text-xs font-display uppercase tracking-widest">
